...
class Face_Recognition_System:
    def __init__(self, root,user_role):
        ...
        self.toggle_btn = Button(self.root, text="✖", font=("Helvetica", 12, "bold"), fg="white", bg="#16161a",  activebackground="#34495E", activeforeground="white", bd=0, command=self.toggle_sidebar)
        self.toggle_btn.pack(pady=10, anchor="w", padx=20)

        # Sidebar (Navigation Bar)
        self.sidebar = Frame(self.root, bg="#16161a", width=300)
        ...
        logo_label = Label(self.sidebar, text="FRAS", font=("roboto", 25, "bold"), fg="white", bg="#16161a", image=self.logo_photo, compound=LEFT)
        ...
        if self.user_role=="teacher":
            ...
            self.create_nav_button("Dashboard", self.show_dashboard, 1)
            self.create_nav_button("Student Details", self.student_details, 2)
            self.create_nav_button("Train Data", self.train_data, 3)
            self.create_nav_button("Help Desk", self.help_data, 4)
        
        self.create_nav_button("Exit", self.iExit, 8)    
  
        # Main content area (dynamic content)
        self.content_frame = Frame(self.root, bg="#010101")
        ...
        # Initialize clock
        self.clock_label = Label(self.root, font=("Helvetica", 20, "bold"), fg="white", bg="#16161a")
    ...
    def create_nav_button(self, text, command, index, icon_path=None):
        def on_enter(e):
            button.config(bg="#7f5af0", fg="white")  # Highlight on hover

        def on_leave(e):
            button.config(bg="white", fg="#010101")  # Revert to default
    ...
    # Clear content frame before adding new widgets
        ...
        img_path = r"C:\Users\HP\Desktop\facial_recognition system\college_images\student11.jpg"
        ...
        stats_frame.place(x=50, y=560, width=1260, height=135)
        if self.user_role=="teacher":
            stats = [
        ...
        for i, (label, value, icon_path) in enumerate(stats):
            ...
            stat_frame = Frame(stats_frame, bg="#00214d", bd=0, relief=FLAT)
            stat_frame.place(x=20 + i * 410, y=10, width=400, height=110)

            # Icon Placeholder (replace with actual icons if available)
            icon_label = Label(stat_frame, text="🔹", font=("Helvetica", 30), bg="#00214d", fg="white")
            icon_label.place(x=10, y=25)

            # Stat Label
            text_frame = Frame(stat_frame, bg="#00214d")
            text_frame.place(x=80, y=10, width=310, height=90)

            Label(text_frame, text=label, font=("Poppins", 16, "bold"), fg="#fffffe", bg="#00214d").pack(anchor="w", pady=2)
            Label(text_frame, text=value, font=("Helvetica", 24, "bold"), fg="#FFD700", bg="#00214d").pack(anchor="w", pady=5)

        
       
     # Quick Actions Frame
        action_frame = Frame(bg_img, bg="#fffffe", bd=5, relief=RIDGE)
        action_frame.place(x=50, y=110, width=1250, height=110)
        ...
        if self.user_role=="teacher":
            ...
            actions = [
            #("🎭 Start Recognition", self.face_data, "start_icon.png"),
            ("Manage Details", self.student_details, "manage_icon.png"),
            ("📊 Train Data", self.train_data, "train_icon.png"),
        ]

        for i, (label, command, icon_path) in enumerate(actions):
            # Use icons if available
            button_frame = Frame(action_frame, bg="#7f5af0")
            button_frame.pack(side=LEFT, expand=True, fill=BOTH, padx=10, pady=5)
            
            # Use Label to simulate buttons for rounded corners
            button = Label(
                button_frame, text=label, font=("Helvetica", 16, "bold"),
                fg="white", bg="#7f5af0", relief=RAISED, bd=2, padx=10, pady=5,
                cursor="hand2"
            )
            button.bind("<Enter>", on_enter)
            button.bind("<Leave>", on_leave)
        ...
        shadow_lbl = Label(
            bg_img,
            bg="#7f5af0",  # Slightly darker shade of the main background
        )
        ...
        title_lbl = Label(
            ...
            bg="#16161a",  # Dark blue background
            fg="white", 
        ...
        title_lbl.place(x=-120, y=0, width=1630, height=80)
