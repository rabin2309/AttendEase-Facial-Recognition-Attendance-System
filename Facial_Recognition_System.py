...
class Login_Window:
    def __init__(self,root):
        ...
        login_frame.place(x=150, y=100, width=300, height=550)
       ...
       # Username Label and Entry
        ...
        # Eye Toggle Button
        self.toggle_button = Button(login_frame, text="👁",font=(12), command=self.toggle_password, bg="white", border=0)
        self.toggle_button.place(x=240, y=210)  # Adjusted placement for alignment

        # Role Dropdown
        role_label = Label(login_frame, text="Role:", font=("Arial", 15), bg="white", fg="#555")
        role_label.pack(pady=10)
        self.var_role = StringVar()
        role_dropdown = ttk.Combobox(login_frame, textvariable=self.var_role, font=("Arial", 12), state="readonly")
        role_dropdown["values"] = ("Select", "student", "teacher")
        role_dropdown.pack(pady=5, ipadx=5, ipady=5)
    ...
    def login(self):
        ...
        # Bypass role selection for admin
        if self.var_email.get() == "admin@gmail.com" and self.var_password.get() == "admin@123":
            self.engine.say("Welcome Admin to the Dashboard.")
            self.engine.runAndWait()
            self.new_window = Toplevel(self.root)
            self.app = AdminDashboard(self.new_window)  # Pass the new window to the admin dashboard
            self.root.withdraw()  # Hide the current login window
            return  # Exit the function
    
        # Check if the role is selected for non-admin users
        if self.var_role.get() == "Select":
            self.engine.say("Please select a role.")
            self.engine.runAndWait()
        ...
        try:
            ...
            if row is None:
                ...
                user_password = row[6]  # Password
                user_role = row[-2]  # Role
                user_status = row[-1]  # Status
                user_name = row[0]  # First Name
                selected_role = self.var_role.get()
    
                if user_status == "decline":
                    self.engine.say("You are not approved. Please contact the admin.")
                    self.engine.runAndWait()
                    messagebox.showerror("Error", "You are not approved. Please contact the admin.")
    
                elif user_status == "approve":
                    if user_role == "student" and selected_role == "student":
                        self.engine.say(f"Welcome, {user_name}, to the Facial Recognition Attendance System.")
                        self.engine.runAndWait()
                        self.new_window = Toplevel(self.root)
                        self.app = Face_Recognition_System(self.new_window, "student")
                        self.root.withdraw()
                    elif user_role == "teacher" and selected_role == "teacher":
                        self.engine.say(f"Welcome, {user_name}, to the Facial Recognition Attendance System.")
                        self.engine.runAndWait()
                        self.new_window = Toplevel(self.root)
                        self.app = Face_Recognition_System(self.new_window, "teacher")
                        self.root.withdraw()
                    else:
                        self.engine.say("Role mismatch. Please select the correct role.")
                        self.engine.runAndWait()
                        messagebox.showerror("Error", "Role mismatch. Please select the correct role.")
                else:
                    messagebox.showerror("Error", "Invalid role or status.")
...
class Register:
    def __init__(self, root):
        ...
        self_combo_security_q = ttk.Combobox(frame, textvariable=self.var_security_q, font=("Aria)l", 12), state="readonly", width=28)
        ...
        role = tk.Label(frame, text="Role:", font=("Arial", 16, "bold"), bg="#f0f8ff", fg="#333")
        role.place(x=370, y=380)
        self_combo_role = ttk.Combobox(frame, textvariable=self.var_role, font=("Arial", 12), state="readonly", width=28)
        self_combo_role["values"] = ("select", "student", "teacher")
        self_combo_role.place(x=370, y=410)
        self_combo_role.current(0)
    ...
    def register_data(self):
        try:
            ...
            if row is not None:
                ...
                my_cursor.execute("INSERT INTO register (fname, lname, contact, email, security_q, security_a, password, role) VALUES (%s, %s, %s, %s, %s, %s, %s,%s)", (
                    self.var_fname.get(),
                    self.var_lname.get(),
